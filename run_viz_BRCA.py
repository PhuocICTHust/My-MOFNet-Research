"""
Visualize omics attention weights — BRCA
Chạy: python run_viz_BRCA.py
Output: BRCA_omics_attention.png  (cùng thư mục với script)
"""
...
def load_attn(ckpt_dir, cancer, seeds):
    ...
    for s in seeds:
        ...
        if 'attention_weights' not in ck:
            ...
            continue
        attn = ck['attention_weights']          # [N, 4]
        if isinstance(attn, torch.Tensor):
            attn = attn.cpu().numpy()
        mean_per_sample = attn.mean(axis=0)     # [4]
        all_attn.append(mean_per_sample)
        loaded.append(s)
        print(f'  seed {s}: {np.round(mean_per_sample, 4)}')
    return np.array(all_attn), loaded           # [n_seeds, 4]

print(f'\n=== {CANCER} omics attention ===')
attn_matrix, loaded_seeds = load_attn(CKPT_DIR, CANCER, SEEDS)

if len(attn_matrix) == 0:
    print('ERROR: no checkpoints loaded. Check CKPT_DIR and CANCER.')
    raise SystemExit(1)

means = attn_matrix.mean(axis=0)   # [4]
stds  = attn_matrix.std(axis=0)    # [4]
